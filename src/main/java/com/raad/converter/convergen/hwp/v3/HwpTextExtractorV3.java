 ...
 */
package com.raad.converter.convergen.hwp.v3;
...
public abstract class HwpTextExtractorV3 {
	...
	public static boolean extractText(InputStream input, Writer writer) throws IOException {
		try {
			try {
				byte[] buf = new byte[HWP_V3_SIGNATURE.length];
				int read = input.read(buf);
				if (read < HWP_V3_SIGNATURE.length) { return false; }
				if (!Arrays.equals(HWP_V3_SIGNATURE, buf)) { return false; }
			} catch (IOException e) {
	...
	private static void extractTextV(InputStream inputStream, Writer writer) throws IOException {
		HwpStreamReader input = new HwpStreamReader(inputStream);
		input.ensureSkip(96);
		int t = input.uint16();
		if (t != 0) { throw new IOException("암호화된 문서는 해석할 수 없습니다"); }
		// 압축 확인
		...
		while (input.available()) {
			if (!writeParaText(input, writer)) { break; }
		}
	}
	...
	private static boolean writeParaText(HwpStreamReader input, Writer writer) throws IOException {
		...
		// # 여기까지 43 bytes
		if (prev_paragraph_shape == 0 && n_chars > 0) { input.ensureSkip(187); }
