package com.raad.converter.comparison;

import com.raad.converter.domain.ExcludedAreas;
import com.raad.converter.domain.ImageComparisonResult;
import com.raad.converter.domain.Rectangle;
...
public class ImageComparison {
    ...
    /**
     * Maximal count of the {@link com.raad.converter.domain.Rectangle}s.
    ...
    /**
     * ExcludedAreas contains a List of {@link com.raad.converter.domain.Rectangle}s to be ignored when comparing images
     */
    ...
    public ImageComparisonResult compareImages() {
        ...
        List<com.raad.converter.domain.Rectangle> rectangles = populateRectangles();
     ...
     *
     * @return the collection of the populated {@link com.raad.converter.domain.Rectangle} objects.
     */
    private List<com.raad.converter.domain.Rectangle> populateRectangles() {
        ...
        groupRegions();
        List<com.raad.converter.domain.Rectangle> rectangles = new ArrayList<>();
        while (counter <= regionCount) {
            com.raad.converter.domain.Rectangle rectangle = createRectangle();
            if (!rectangle.equals(com.raad.converter.domain.Rectangle.createDefault()) && rectangle.size() >= minimalRectangleSize) {
    ...
    /**
     * Create a {@link com.raad.converter.domain.Rectangle} object.
     *
     * @return the {@link com.raad.converter.domain.Rectangle} object.
     */
    private com.raad.converter.domain.Rectangle createRectangle() {
        com.raad.converter.domain.Rectangle rectangle = com.raad.converter.domain.Rectangle.createDefault();
     ...
     */
    private void updateRectangleCreation(com.raad.converter.domain.Rectangle rectangle, int x, int y) {
     ...
     */
    private List<com.raad.converter.domain.Rectangle> mergeRectangles(List<com.raad.converter.domain.Rectangle> rectangles) {
        int position = 0;
        while (position < rectangles.size()) {
            for (int i = 1 + position; i < rectangles.size(); i++) {
                com.raad.converter.domain.Rectangle r1 = rectangles.get(position);
                com.raad.converter.domain.Rectangle r2 = rectangles.get(i);
                if (r1.equals(com.raad.converter.domain.Rectangle.createZero())) {
                    continue;
                }
        ...
        return rectangles.stream().filter(it -> !it.equals(com.raad.converter.domain.Rectangle.createZero())).collect(Collectors.toList());
    }

    /**
     * Draw the rectangles based on collection of the rectangles and result image.
     *
     * @param rectangles the collection of the {@link com.raad.converter.domain.Rectangle} objects.
     * @return result {@link BufferedImage} with drawn rectangles.
     */
    private BufferedImage drawRectangles(List<com.raad.converter.domain.Rectangle> rectangles) {
     ...
     *
     * @param rectangles the collection of the {@link com.raad.converter.domain.Rectangle} of differences.
     * @param graphics   prepared {@link Graphics2D}object.
     */
    private void drawRectanglesOfDifferences(List<com.raad.converter.domain.Rectangle> rectangles, Graphics2D graphics) {
        List<com.raad.converter.domain.Rectangle> rectanglesForDraw;
        graphics.setColor(Color.orange);

        if (maximalRectangleCount > 0 && maximalRectangleCount < rectangles.size()) {
            rectanglesForDraw = rectangles.stream()
                    .sorted(Comparator.comparing(com.raad.converter.domain.Rectangle::size))
    ...
    /**
     * Draw rectangles based on collection of the {@link com.raad.converter.domain.Rectangle} and {@link Graphics2D}.
     ...
     * @param rectangles the collection of the {@link com.raad.converter.domain.Rectangle}.
     */
    private void draw(Graphics2D graphics, List<com.raad.converter.domain.Rectangle> rectangles) {
    ...
    /**
     * Fill rectangles based on collection of the {@link com.raad.converter.domain.Rectangle} and {@link Graphics2D}.
     * getWidth/getHeight return real width/height,
     * so need to draw rectangle fill two px smaller to fit inside rectangle borders.
     *
     * @param graphics       the {@link Graphics2D} object for drawing.
     * @param rectangles     rectangles the collection of the {@link com.raad.converter.domain.Rectangle}.
     * @param percentOpacity the opacity of the fill.
     */
    private void fillRectangles(Graphics2D graphics, List<com.raad.converter.domain.Rectangle> rectangles, double percentOpacity) {
     ...
     *
     * @return List of {@link com.raad.converter.domain.Rectangle}
     */
    public List<com.raad.converter.domain.Rectangle> createMask() {
